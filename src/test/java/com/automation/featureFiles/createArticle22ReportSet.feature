...
Feature: ILO Normes Application Functionality- Create Article 22 report set
  ...
  Scenario: User is logged in as Chief of Branch and signs off as Chief of Branch
    ...
    And user clicks on "HoUSignOffAfterExpertReviews" icon
     ...
     @ILONormes @CreateArticle22ReportSet @HOUsendCommentToLSAndTL
    Scenario: User is logged in as Head of Unit and sent comment to LS and TL
    Given user switched to "Head of Unit" view
    And user enters the report set name in the report set name column
    And user clicks on "createReportSetBtnLink" icon
     And user clicks on "Send comment to Legal Specialist" icon
    And user clicks on "Send" icon
    And user clicks on "Send comment to Thematic Lead" icon
    And user clicks on "Send" icon  
    
    
    @LSClickOnEndTaskAndNotifyHoU
     Scenario: User is logged in as Legal Specialist and click on End task
    Given user switched to "Legal Specialist" view
     And user enters the report set name in the report set name column
    And user clicks on "createReportSetBtnLink" icon
    And user clicks on "End task and notify HoU" icon
    And user clicks on "yes" icon
    
    @TLClickOnEndTaskAndNotifyHoU
     Scenario: User is logged in as Thematic Lead and click on End task
    Given user switched to "Thematic Lead" view
     And user enters the report set name in the report set name column
    And user clicks on "createReportSetBtnLink" icon
    And user clicks on "End task and notify HoU" icon
    ...
    @ExpertReviewISNoInStep2
    Scenario: User is logged as Head of Unit and gives the Yes review 
    Given user switched to "Head of Unit" view
    And user enters the report set name in the report set name column
    And user clicks on "createReportSetBtnLink" icon
    And user clicks on "noReviewForCEACRExpert" icon
    And user clicks on "save" icon 
    
      @demo111
    Scenario: User is logged in as Head of Unit and sent comment to LS and TL
    Given user switched to "Head of Unit" view
     And user clicks on "dlc drafting" icon
